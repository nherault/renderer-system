...
export function renderText(_a) {
    ...
    if (currentRenderer.text !== undefined) {
        ...
        var newPositionWithAnchor = computeOrDefault(currentRenderer.anchor, function (anchor) { return computePositionWithAnchor(function (target) { return targetRetriever({ target: target, currentObject: currentObject, view: view }); }, anchor); }, position);
        var newPositionWithOffset = computePositionWithOffset(newPositionWithAnchor, getOrDefault(currentRenderer.offsetPosition, { x: 0, y: 0 }));
...
function renderSizeText(context, currentRenderer, position, size, lineHeight) {
    var currLineHeight = getOrDefault(lineHeight, getLineHeight(context) * 1.2);
    var lines = currentRenderer.text.split('\n');
    var _loop_1 = function (i) {
        var line = lines[i];
        computeStrokeFill(currentRenderer, function () { return context.strokeText(line, position.x, position.y + currLineHeight * i, size.x); }, function () { return context.fillText(line, position.x, position.y + currLineHeight * i, size.x); });
    };
        ...
        if (maxCharacterByLine !== undefined) {
            ...
            for (var j = 1; j < words.length; j++) {
                if (line.length + words[j].length > maxCharacterByLine) {
                    computeStrokeFill(currentRenderer, function () { return context.strokeText(line, position.x, position.y + currLineHeight * lineCount); }, function () { return context.fillText(line, position.x, position.y + currLineHeight * lineCount); });
                    lineCount++;
                    line = '';
                }
                line += words[j] + ' ';
