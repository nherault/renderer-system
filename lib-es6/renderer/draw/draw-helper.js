...
var DrawHelper = /** @class */ (function () {
    ...
    DrawHelper.translateIntoView = function (view, source, coordx, coordy) {
        var x = coordx;
        var y = coordy;
        if (!DrawHelper.isInView(view, ShapeFactory.createRectangle(coordx, coordy, source.width, source.height))) {
    ...
    DrawHelper.drawFromCanvasRepeatX = function (context, view, source, coordx, coordy, isTranslate) {
        ...
        if (DrawHelper.isInView(view, ShapeFactory.createRectangle(x, y, source.width, source.height))) {
            context.drawImage(source, x, y);
            // Draw before
            var currCoordX = x - source.width;
            while (DrawHelper.isInView(view, ShapeFactory.createRectangle(currCoordX, y, source.width, source.height))) {
    ...
    DrawHelper.drawFromCanvasRepeatY = function (context, view, source, coordx, coordy, isTranslate) {
        ...
        if (DrawHelper.isInView(view, ShapeFactory.createRectangle(x, y, source.width, source.height))) {
            ...
            while (DrawHelper.isInView(view, ShapeFactory.createRectangle(x, currCoordY, source.width, source.height))) {
    ...
    DrawHelper.drawFromResource = function (displayReferences, resourceId, rectangle) {
        ...
        if (result != null) {
            Canvas2DDrawHelper.drawFromCanvasClippingAndScale(displayReferences.context, displayReferences.resourceStore.getResource(resourceId), result.coord.x, result.coord.y, result.rect);
        }
        return this;
    };
    ...
    DrawHelper.calculateRectangleInsideView = function (view, rectangle) {
        ...
        var resultClipping = ShapeFactory.createRectangle(0, 0, rectangle.size.x, rectangle.size.y);
