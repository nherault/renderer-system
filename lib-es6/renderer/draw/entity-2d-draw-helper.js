...
var Entity2DDrawHelper = /** @class */ (function () {
        ...
        if (source) {
            if (entities instanceof Array) {
                entities.forEach(function (entity) {
                    DrawHelper.drawFromSpriteAndView(displayReferences.context, displayReferences.view, entity, source);
                });
            }
            else {
                DrawHelper.drawFromSpriteAndView(displayReferences.context, displayReferences.view, entities, source);
            }
        }
        return this;
    };
    ...
    Entity2DDrawHelper.drawEntity2D = function (context, view, entity, source, sprite) {
        if (entity.rotation !== undefined && entity.rotation !== null && entity.rotation !== 0) {
            ...
            DrawHelper.drawFromSpriteAndView(context, view, entity, source, sprite);
        }
    };
    ...
    */
    ...
    Entity2DDrawHelper.rotateImage = function (context, entity, source, sprite) {
        ...
        context.rotate(rad);
        // draw the image
        DrawHelper.drawFromSpriteAndScale(context, source, sprite, -diagonal / 2, -diagonal / 2, entity.size.x, entity.size.y);
        // reset the canvas
        context.restore();
    };
    // TODO: VIEW SCALE
    ...
    Entity2DDrawHelper.flipImage = function (context, entity, source, sprite, isTranslateObject) {
        ...
        DrawHelper.drawFromSpriteAndScale(context, source, sprite, posX, posY, entity.size.x, entity.size.y);
    ...
    Entity2DDrawHelper.flipAndRotateImage = function (context, entity, source, sprite) {
        var tempCanvasFlip = CanvasFactory.createCanvas(entity.size.x, entity.size.y);
        var tempContextFlip = tempCanvasFlip.getContext('2d');
        if (tempContextFlip) {
            Entity2DDrawHelper.flipImage(tempContextFlip, entity, source, sprite, false);
        }
        var diagonal = Math.sqrt(Math.pow(entity.size.x, 2) + Math.pow(entity.size.y, 2));
        var tempCanvasRotate = CanvasFactory.createCanvas(diagonal, diagonal);
        ...
        if (tempContextRotate) {
            ...
            Canvas2DDrawHelper.drawFromCanvas(tempContextRotate, tempCanvasFlip, x, y);
            var x1 = entity.position.x;
            var y1 = entity.position.y;
            Canvas2DDrawHelper.drawFromCanvas(context, tempCanvasRotate, x1, y1);
        }
    };
    Entity2DDrawHelper.drawWithResourceIdFinder = function (entityToDraw, displayReferences, resourceIdFinder) {
        ...
        var source = displayReferences.resourceStore.getResource(resourceId);
        if (source) {
            DrawHelper.drawFromSpriteAndView(displayReferences.context, displayReferences.view, entityToDraw, source);
        }
    };
    return Entity2DDrawHelper;
}());
export { Entity2DDrawHelper };
