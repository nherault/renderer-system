...
var animation_factory_1 = require("../tiles/animation-factory");
var animation_handler_1 = require("../tiles/animation-handler");
var scene_types_1 = require("./scene.types");
var SceneHandler = /** @class */ (function () {
    ...
    SceneHandler.getFirstObjectByProperty = function (sceneData, propertyName, propertyValue) {
        var objectLayers = sceneData.layers.filter(function (layer) { return layer.type === scene_types_1.LayerType.OBJECT_LAYER; });
    ...
    SceneHandler.getObjectLayers = function (sceneData) {
        return sceneData.layers
            .filter(function (layer) { return layer.type === scene_types_1.LayerType.OBJECT_LAYER; });
    };
    ...
    SceneHandler.updateObjectAnimation = function (sceneData, delta, layer) {
        layer.objects.forEach(function (currentObject) {
            if (currentObject && currentObject.animation) {
                ...
                if (frames_1) {
                    animation_handler_1.AnimationHandler.updateAnimation({ state: currentObject.animation, frames: frames_1 }, delta);
                }
            }
        });
    };
    ...
    SceneHandler.addObjectToLayerFromResourceId = function (sceneData, layerId, position, resourceId) {
        ...
        if (tileset) {
            if (resource.type === scene_types_1.ResourceType.IMAGE) {
                objectData.size = {
                    x: tileset.imageWidth,
                    y: tileset.imageHeight,
                };
            }
            else if (resource.type === scene_types_1.ResourceType.TILE) {
                objectData.size = {
                    x: tileset.tileWidth,
                    y: tileset.tileHeight,
                };
            }
            else if (resource.type === scene_types_1.ResourceType.ANIMATION) {
                objectData.size = {
                    x: tileset.tileWidth,
                    y: tileset.tileHeight,
                };
                ...
                if (animationFrameData) {
                    objectData.animation = animation_factory_1.AnimationFactory.createAnimationState(animationFrameData, { isLoop: true });
                }
            }
        }
    ...
    SceneHandler.getCollisionMaskFromObject = function (sceneData, objectData, generateDefault) {
        ...
        if (resource.type === scene_types_1.ResourceType.ANIMATION) {
            ...
            if (frames_2 && objectData.animation) {
                var tileIndex = animation_handler_1.AnimationHandler.getCurrentId({ state: objectData.animation, frames: frames_2 });
                var tileset = SceneHandler.getTileset(sceneData, resource.tilesetId);
    ...
    SceneHandler.getImageCollisionMaskFromTileset = function (tileset) {
        return {
            height: tileset.type === scene_types_1.TilesetType.IMAGE ? tileset.imageHeight : tileset.tileHeight,
            rotation: 0,
            type: 'rectangle',
            visible: true,
            width: tileset.type === scene_types_1.TilesetType.IMAGE ? tileset.imageWidth : tileset.tileWidth,
            x: 0,
            y: 0,
        };
    };
